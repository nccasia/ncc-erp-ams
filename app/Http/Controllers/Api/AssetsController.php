...
class AssetsController extends Controller
{
    ...
    public function index(Request $request, $audit = null)
    {
        ...
        if (Route::currentRouteName() == 'api.depreciation-report.index') {
            ...
            $transformer =
                'App\Http\Transformers\DepreciationReportTransformer';
        ...
        $assets = Company::scopeCompanyables(
            Asset::select('assets.*'),
            'company_id',
            'assets'
        )->with(
        ...
        if ($filter_non_deprecable_assets) {
            $non_deprecable_models = AssetModel::select('id')
        ...
        foreach ($all_custom_fields as $field) {
            if ($request->filled($field->db_column_name())) {
                $assets->where(
                    ...
                    '=',
                    $request->input($field->db_column_name())
                );
            }
        }
        ...
        if ($request->filled('dateFrom', 'dateTo')) {
            $assets->whereBetween('assets.purchase_date', [
        ...
        if ($request->filled('rtd_location_id')) {
            $assets->where(
                'assets.rtd_location_id',
                '=',
                $request->input('rtd_location_id')
            );
        }
        ...
        $offset =
            $assets && $request->get('offset') > $assets->count()
                ? $assets->count()
        ...
        switch ($request->input('status')) {
            case 'Deleted':
                ...
                break;
            case 'Pending':
                $assets->join('status_labels AS status_alias', function (
                    $join
                ) {
                    $join
                        ->on('status_alias.id', '=', 'assets.status_id')
        ...
        if ($request->notRequest == 1) {
            $assets = $assets
                ->with('finfast_request_asset')
                ->doesntHave('finfast_request_asset');
        }
    ...
    public function showByTag(Request $request, $tag)
    {
        ...
        return response()->json(
            Helper::formatStandardApiResponse('error', null, 'Asset not found'),
            200
        );
    }
    ...
    public function showBySerial(Request $request, $serial)
    {
        ...
        if (
            ...
            return (new AssetsTransformer())->transformAssets(
                $assets,
                $assets->count()
            );
        }
    ...
    public function show(Request $request, $id)
    {
        if (
            ...
            return (new AssetsTransformer())->transformAsset(
                $asset,
    ...
    public function licenses(Request $request, $id)
    {
        ...
        return (new LicensesTransformer())->transformLicenses(
            $licenses,
            $licenses->count()
        );
    }
    ...
    public function selectlist(Request $request)
    {
        ...
        if (
            $request->filled('assetStatusType') &&
            $request->input('assetStatusType') === 'RTD'
        ) {
            $assets = $assets->RTD();
        }
        ...
        foreach ($assets as $asset) {
            ...
            $asset->use_image = $asset->getImageUrl()
                ? $asset->getImageUrl()
                : null;
        }
    ...
    public function store(ImageUploadRequest $request)
    {
        ...
        $asset
            ->model()
            ->associate(AssetModel::find((int) $request->get('model_id')));

        $asset->name = $request->get('name');
        $asset->serial = $request->get('serial');
        $asset->company_id = Company::getIdForCurrentUser(
            $request->get('company_id')
        );
        $asset->model_id = $request->get('model_id');
        $asset->order_number = $request->get('order_number');
        $asset->notes = $request->get('notes');
        $asset->asset_tag = $request->get(
            'asset_tag',
        ...
        $asset->purchase_cost = Helper::ParseCurrency(
            $request->get('purchase_cost')
        ); // this is the API's store method, so I don't know that I want to do this? Confusing. FIXME (or not?!)
        ...
        if ($model && $model->fieldset) {
            foreach ($model->fieldset->fields as $field) {
                ...
                if ($field_val == null) {
                    ...
                    $field_val = $field->defaultValue(
                        $request->get('model_id')
                    );
                    \Log::debug(
                        'Use the default fieldset value of ' .
                            $field->defaultValue($request->get('model_id'))
                    );
                }
                ...
                if ($field->field_encrypted == '1') {
                    \Log::debug(
                        'This model field is encrypted in this fieldset.'
                    );
                    ...
                    if (Gate::allows('admin')) {
                        ...
                        if (
                            ...
                            $field_val = \Crypt::encrypt(
                                $field->defaultValue($request->get('model_id'))
                            );
                        } else {
                            $field_val = \Crypt::encrypt(
                                $request->input($field->convertUnicodeDbSlug())
                            );
                        }
                    }
                }
    ...
    public function update(ImageUploadRequest $request, $id)
    {
        ...
        if ($asset = Asset::find($id)) {
            ...
            $request->filled('model_id')
                ? $asset
                    ->model()
                    ->associate(AssetModel::find($request->get('model_id')))
                : null;
            ...
            if ($request->has('image_source')) {
                $request->offsetSet(
                    'image',
                    $request->offsetGet('image_source')
                );
            }
            ...
            if (
                ...
                isset($model->fieldset)
            ) {
                foreach ($model->fieldset->fields as $field) {
                    if ($request->has($field->convertUnicodeDbSlug())) {
                        if ($field->field_encrypted == '1') {
                            if (Gate::allows('admin')) {
                                $asset->{$field->convertUnicodeDbSlug()} = \Crypt::encrypt(
                                    $request->input(
                                        $field->convertUnicodeDbSlug()
                                    )
                                );
                            ...
                            $asset->{$field->convertUnicodeDbSlug()} = $request->input(
                                $field->convertUnicodeDbSlug()
                            );
                        }
                    }
            ...
            if (
                $user &&
                $assigned_status !== $request->get('assigned_status')
            ) {
            ...
            if ($asset->save()) {
                if (
                    ...
                    Asset::where('assigned_type', \App\Models\Asset::class)
                        ->where('assigned_to', $id)
                        ->update(['location_id' => $target->location_id]);
                } elseif (
                    $request->filled('assigned_location') &&
                    ($target = Location::find(
                        $request->get('assigned_location')
                    ))
                ) {
                    $location = $target->id;
                }
        ...
        if (isset($asset->id)) {
            ...
            return response()->json(
                Helper::formatStandardApiResponse(
                    'success',
                    null,
                    trans('admin/hardware/message.restore.success')
                )
            );
        }
        return response()->json(
    ...
    public function checkout(AssetCheckoutRequest $request, $asset_id)
    {
        ...
        if (request('checkout_to_type') == 'location') {
            ...
            $target = Asset::where('id', '!=', $asset_id)->find(
                request('assigned_asset')
            );
            $asset->location_id = $target->rtd_location_id;
            // Override with the asset's location_id if it has one
            $asset->location_id =
                $target && isset($target->location_id)
                    ? $target->location_id
                    : '';
        ...
        if (!isset($target)) {
            return response()->json(
                Helper::formatStandardApiResponse(
                    'error',
                    $error_payload,
                    'Checkout target for asset ' .
                        e($asset->asset_tag) .
                        ' is invalid - ' .
                        $error_payload['target_type'] .
                        ' does not exist.'
                )
            );
        }
    ...
    public function audit(Request $request)
    {
        ...
        $settings = Setting::getSettings();
        $dt = Carbon::now()
            ->addMonths($settings->audit_interval)
            ->toDateString();
                ...
                return response()->json(
                    Helper::formatStandardApiResponse(
                        ...
                        [
                            ...
                            'next_audit_date' => Helper::getFormattedDateObject(
    ...
    public function requestable(Request $request)
    {
        ...
        return (new AssetsTransformer())->transformRequestedAssets(
            $assets,
            $total
        );
    }
    ...
    public function clone(Request $request)
    {
        ...
        $new_asset->asset_tag =
            $new_asset->asset_tag . Carbon::now()->toDateTimeString();
