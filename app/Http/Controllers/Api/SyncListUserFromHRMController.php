...
class SyncListUserFromHRMController extends Controller
{
    ...
    {
        $response = Http::get('http://hrm-api.nccsoft.vn/api/services/app/Public/GetUserForCheckin');
        $response = json_decode($response);
        if ($response == null || !is_array($response->result))
            return response()->json(Helper::formatStandardApiResponse('error', null, 'bug'));
