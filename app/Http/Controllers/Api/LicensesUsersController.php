...
class LicensesUsersController extends Controller
{
    ...
    public function showUsersLicense(Request $request, $licenseId)
    {
        if (SoftwareLicenses::find($licenseId)) {
            ...
            [$licenseUsers, $total] = $this->getUsers($request, $licenseUsers);
            if ($licenseUsers) {
    ...
    /**
     * Get the list users have checked out
     *
     * @param Request $request
     * @param licenseUsers $licenseUsers
     * @return array
     */
    public function getUsers(Request $request, $licenseUsers){
